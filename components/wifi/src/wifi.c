...
static void ip_event_cb(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    case IP_EVENT_STA_GOT_IP:
        ...
        wifi_context_update_ip(&event->ip_info);

        // Update RSSI and other connection info
        wifi_ap_record_t ap_info;
        if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK)
        {
            wifi_context_update_connection((char *)ap_info.ssid, ap_info.rssi);
        }
    ...
    case (IP_EVENT_GOT_IP6):
        ...
        wifi_context_update_ip6(&event_ip6->ip6_info.ip);
        wifi_retry_count = 0;
        xEventGroupSetBits(s_wifi_event_group, WIFI_CONNECTED_BIT);
        break;
    default:
...
static void wifi_event_cb(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    case WIFI_EVENT_STA_CONNECTED:
    {
        ...
        wifi_context_update_state(WIFI_CONNECTED);
        wifi_context_update_connection((char *)event->ssid, 0); // RSSI will be updated in IP event
        break;
    }
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
    {
        ...
        wifi_context_update_state(WIFI_DISCONNECTED);
        wifi_context_update_disconnect_reason(disconn->reason);
